...
def main(cmd_line):
	...
	# 10 GeV to 1 TeV — 60 steps (~0.053 dec/step, fine resolution)
	Eb_arraylog = np.logspace(np.log10(10000), np.log10(1000000), 60)
	...
	for i in range(len(Eb_arraylog)):
	    ...
	    BPL_like=BPLfit['loglike']
	    # Save SED image for this break energy step (no roi file written)
	    gta.sed(srcname, loge_bins=log_energy_bins, free_radius=3, make_plots=True)
	    sed_src = "%s/output/4fgl_j%s_sed.png" % (outdir, srcname[5:16])
	    sed_dst = "%s/output/sed_BPL_eb%.0fMeV_%s.png" % (outdir, Eb_arraylog[i], file_tag)
	    
	    if os.path.exists(sed_src):
	        os.rename(sed_src, sed_dst)
	    os.system("rm -rf %s/output/4fgl_j%s_sedlnl.png" % (outdir, srcname[5:16]))
	    os.system("rm -rf %s/output/4fgl_j%s_sed.fits" % (outdir, srcname[5:16]))
